...
// Mace modified this file to support Mace multi-threading.
...
class MultiThreadGemmContext : public MultiThreadGemmContextBase {
 public:
  MultiThreadGemmContext() {}

#ifdef GEMMLOWP_USE_MACE_THREAD_POOL
  MultiThreadGemmContext(mace::utils::ThreadPool *thread_pool)
...
#ifdef GEMMLOWP_USE_MACE_THREAD_POOL
  mace::utils::ThreadPool *thread_pool_;
#endif
};
...
void MultiThreadGemm(GemmContextType* context, const KernelBase& kernel,
  ...
  //std::cout << "loop over large blocks of the RHS" << std::endl;
  for (int c = 0; c < cols; c += block_params.l2_cols) {
...
#ifdef GEMMLOWP_USE_MACE_THREAD_POOL
    //std::cout << "give work to MACE thread pool" << std::endl;
    const std::vector<std::unique_ptr<Allocator>> &allocator_pool =
        context->allocator_pool();

    mace::utils::ThreadPool *thread_pool = context->thread_pool();
    ...
    thread_pool->Compute1D([=,
      ...
      for (int64_t n = start; n < end; n += step) {
        ...
        int next_start_row = std::min(
          rows, RoundUp<KernelFormat::kRows, int>(
              rows * (n + 1) / task_count));
        ...
        task.Run();
        //std::cout << "finish task " << start << std::endl;
      }
    }, 0, task_count, 1);
#else
