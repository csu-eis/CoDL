...
TEST_F(ConcatOpTest, OPENCLNoiseReductionModel) {
  OpenCLRandomTest<float>({{1, 1, 161, 1}, {1, 1, 161, 1}}, 3, 1);
  OpenCLRandomTest<float>({{1, 1, 1, 2}, {1, 1, 1, 1}}, 3, 1);
  std::vector<std::vector<index_t>> shapes;
  for (int i = 0; i < 256; i ++) {
    shapes.push_back({1, 1, 9, 1});
  }
  OpenCLRandomTest<float>(shapes, 3, 1);
}

TEST_F(ConcatOpTest, OPENCLHalfNoiseReductionModel) {
  OpenCLRandomTest<half>({{1, 1, 161, 1}, {1, 1, 161, 1}}, 3, 1);
  OpenCLRandomTest<half>({{1, 1, 1, 2}, {1, 1, 1, 1}}, 3, 1);
  std::vector<std::vector<index_t>> shapes;
  for (int i = 0; i < 256; i ++) {
    shapes.push_back({1, 1, 9, 1});
  }
  OpenCLRandomTest<half>(shapes, 3, 1);
}
