...
// NOTE(fucheng): Add CPUGPU device type
enum DeviceType { CPU = 0, GPU = 2, HEXAGON = 3, HTA = 4, APU = 5, CPUGPU = 6 };
...
enum GPUMemoryType {
  GPU_MEM_TYPE_IMAGE = 0,
  GPU_MEM_TYPE_BUFFER = 1
};
...
constexpr float kRatioFromFunction   = 3.0f;
constexpr float kRatioFromConfigFile = 4.0f;
constexpr float kRatioTuning         = 5.0f;
constexpr float kRatioFromPredictor  = 6.0f;
constexpr float kRatioGpuMinimum     = 7.0f;
...
enum PartitionDim {
  ...
  DIM_INPUT_WIDTH = 2,
  DIM_INPUT_CHANNEL = 3,
  DIM_OUTPUT_CHANNEL = 4,
};

class PartitionRunConfig {
public:
  PartitionRunConfig(const float ratio)
    : do_pr_configer_reload_(false),
      ratio_(ratio),
      dim_type_(PartitionDim::DIM_NONE) {}
  
  PartitionRunConfig(const float ratio, const PartitionDim dim_type)
    : do_pr_configer_reload_(false),
      ratio_(ratio),
      dim_type_(dim_type) {}

  PartitionRunConfig(const float ratio,
                     const PartitionDim dim_type,
                     const bool do_reload)
    : do_pr_configer_reload_(do_reload),
      ratio_(ratio),
      dim_type_(dim_type) {}

  inline void set_do_pr_configer_reload(bool do_reload) {
    do_pr_configer_reload_ = do_reload;
  }

  inline bool do_pr_configer_reload() const {
    return do_pr_configer_reload_;
  }
  ...
  inline PartitionDim dim_type() const {
    return dim_type_;
  }

private:
  bool do_pr_configer_reload_;
  float ratio_;
  PartitionDim dim_type_;
};

/**
 * ADD(fucheng): MACE Runtime Statistics
 **/
template<typename T>
class MaceValueStatistics {
public:
  MaceValueStatistics() : acc_value_(0) {}
  virtual ~MaceValueStatistics() = default;

  inline void Accumulate(const T v) {
    acc_value_ += v;
  }
...
class MaceMACsStatistics : public MaceValueStatistics<int64_t> {
public:
  MaceMACsStatistics() : MaceValueStatistics() {}

  static int64_t Compute(const std::string &op_type,
                         const std::vector<int64_t> &kernel_shape,
                         const std::vector<int64_t> &output_shape);
};

class MaceFLOPsStatistics : public MaceValueStatistics<int64_t> {
public:
  MaceFLOPsStatistics() : MaceValueStatistics() {}
...
struct RuntimeStatConfig {
  bool is_macs_enabled;
  bool is_flops_enabled;
};

class MaceRuntimeStatistics {
public:
  MaceRuntimeStatistics() : macs_stat_(nullptr) {}

  void Init(const RuntimeStatConfig *config);

  inline MaceMACsStatistics *macs_stat() {
    return macs_stat_.get();
  }

  inline MaceFLOPsStatistics *flops_stat() {
    return flops_stat_.get();
  }

private:
  std::unique_ptr<MaceMACsStatistics> macs_stat_;
  std::unique_ptr<MaceFLOPsStatistics> flops_stat_;
};
...
class MACE_API MaceEngineConfig {
 ...
 public:
  ...
  //bool is_runtime_stat_enabled() const;

  MaceStatus SetGPUPrecisionHint(int gpu_precision_hint);

  //int gpu_precision_hint() const;
