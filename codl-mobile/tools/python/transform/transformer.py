...
class Transformer(base_converter.ConverterInterface):
    ...
    def run(self):
        for key in self._option.transformer_option:
            print('Transform option key %s' % key)
            ...
            for input_node in self._option.input_nodes.values():
                ...
                for op in self._model.op:
                    if input_node.name in op.output:
                        ...
                        break
                print('input_node_name {}, existed {}'.format(input_node.name, input_node_existed))
                ...
                if elt_type == EltwiseType.SQR_DIFF.value and\
                    ...
                    if consumer_op.type == MaceOp.Reduce.name:
                        ...
                        if reduce_type == ReduceType.MEAN.value and\
                                ...
                                len(axis) == 2 and axis[0] == 1 and axis[1] == 2 and\
                                keep_dims > 0:
    ...
    def sort_by_execution(self):
        ...
        for op in net.op:
            if op.type not in [MaceOp.Quantize.name, MaceOp.Dequantize.name]:
                ...
                index_str = ''
            print("%s (%s, index:%s, df:%s): %s" % (
                    op.name, op.type, index_str, ConverterUtil.data_format(op),
    ...
    def transpose_data_format(self):
        ...
        for op in net.op:
            has_data_format = ConverterUtil.data_format(op) == \
                              DataFormat.AUTO
            print('Transpose data format (%d/%d, src_df %s, has_df %s)' \
                    % (op_idx, len(net.op), src_data_format.name, str(has_data_format)))
            op_idx = op_idx + 1
            # transpose args
