...
class OnnxConverter(base_converter.ConverterInterface):
    ...
    def __init__(self, option, src_model_file):
        self._op_converters = {
            ...
            OnnxOpType.ConstantOfShape.name: self.convert_constant_of_shape,
        ...
        if polish_available and hasattr(onnx.utils, "polish_model"):
            ...
            onnx_model = self.polish_model(onnx_model)
    ...
    def infer_shapes(self):
        ...
        for input in graph_def.input:
            ...
            # DEBUG(fucheng): Check input name.
            print('input name {}'.format(input.name))
            #print('option input node {}'.format(self._option.input_nodes))
            mace_check(
    ...
    def transofrm_axis_nchw_to_nhwc(self, axis, input_tensor):
        print('axis {}'.format(axis))
        is_list = isinstance(axis, list)
        if is_list:
            mace_check(len(axis) == 1, "Only support one-size axis.")
            is_list = True
            axis = axis[0]
        mace_check(axis >= 0, "axis must be >= 0.")
        
        if input_tensor not in self._graph_shapes_dict:
            #mace_check(False, "input tensor has no shape.")
            # NOTE(fucheng): Three dimensions by default.
            shape_info = (1, 1, 1)
        else:
            shape_info = self._graph_shapes_dict[input_tensor]
        shape_len = len(shape_info)
        print('shape_info {}, shape_len {}'.format(shape_info, shape_len))
        ...
        if shape_len == 1:
            ...
            mace_check(False, "Unsupported shape length: " + str(shape_len))

        trans_axis = trans_arr[axis]
        print('trans_axis {}'.format(trans_axis))
        if is_list:
            return [trans_axis]
        else:
            return trans_axis
    ...
    def run(self):
        # DEBUG(fucheng): Do not do infer shape.
        #self.infer_shapes()
        graph_def = self._onnx_model.graph
    ...
    def add_stride_pad_kernel_arg(self, attrs, op_def):
        if 'strides' in attrs:
            ...
            mace_check(len(strides) == 2 or len(strides) == 3,
                       "strides should has 2 or 3 values.")
            if len(strides) == 2:
        ...
        if 'kernel_shape' in attrs:
            ...
            mace_check(len(kernel_shape) == 2 or len(kernel_shape) == 3,
                       "kernel shape should has 2 or 3 values.")
        ...
        if 'output_padding' in attrs:
            output_pads = attrs['output_padding']
            output_pad = [output_pads[0], output_pads[1]]
            output_padding_arg = op_def.arg.add()
            output_padding_arg.name = MaceKeyword.mace_output_padding_values_str
            output_padding_arg.ints.extend(output_pad)
    ...
    def convert_ops(self, graph_def):
        for n in graph_def.node:
            node = OnnxNode(n)
            if node.op_type not in self._op_converters:
                print('Onnx op type %s (%s) is not suppoted yet' % (node.name, node.op_type))
        ...
        for n in graph_def.node:
            ...
            #print('Convert onnx op type %s (%s)' % (node.name, node.op_type))
    ...
    def convert_concat(self, node):
        ...
        if self._isKaldi is False:
            ...
            axis_value = -1
        if self._option.device == DeviceType.GPU.value or \
           self._option.device == DeviceType.CPU_GPU.value:
            axis_value = self.transofrm_axis_nchw_to_nhwc(
                    axis_value, node.inputs[0])
        axis_arg = op.arg.add()
        axis_arg.name = MaceKeyword.mace_axis_str
        axis_arg.i = axis_value
    ...
    def convert_conv2d(self, node):
        ...
        if is_depthwise:
            ...
            #print('len_kernel_shape: {}'.format(len(kernel_shape)))
            mace_check(len(kernel_shape) == 2 or len(kernel_shape) == 3,
                       'Mace only supports Conv2D or Conv3D')
            if len(kernel_shape) == 2:
    ...
    def convert_eltwise(self, node):
        ...
        inputs_num = len(node.inputs)
        #print('ConvertEltwise: op_type {}, inputs_len {}'.format(
        #        node.op_type, len(node.inputs)))
        if node.op_type == OnnxOpType.Sqrt.name:
            ...
            #print('inputs_num: {}'.format(inputs_num))
    ...
    def convert_shape(self, node):
        ...
        #print('op_type {}, device {}'.format(op.type, self._option.device))
        if self._option.device == DeviceType.GPU.value or \
           self._option.device == DeviceType.CPU_GPU.value:
            #print('DT_HALF')
            op.output_type.extend([mace_pb2.DT_HALF])
        else:
            #print('DT_INT32')
            op.output_type.extend([mace_pb2.DT_INT32])
        #print('output_type {}'.format(op.output_type))
    ...
    def convert_slice(self, node):
        ...
        if 'starts' in node.attrs:
            ...
            if 'axes' in node.attrs:
                print('starts {}, ends {}, axes {}'.format(starts, ends, axes))
    ...
    def convert_split(self, node):
        ...
        if 'axis' in node.attrs:
            ...
            if self._option.device == DeviceType.GPU.value or \
               self._option.device == DeviceType.CPU_GPU.value:
                value = self.transofrm_axis_nchw_to_nhwc(
                        value, node.inputs[0])
        else:
            value = 0
        axis_arg = op.arg.add()
    ...
    def convert_constant_of_shape(self, node):
        op = self.convert_general_op(node)
        op.type = MaceOp.ConstantOfShape.name

        onnx_tensor = node.attrs['value']
        tensor_value = numpy_helper.to_array(onnx_tensor)
        data_type = onnx_dtype(onnx_tensor.data_type)

        #print('tensor_value: {}'.format(tensor_value))
        ...
        if data_type == np.float32 or data_type == np.float64:
            ...
            value_arg.f = tensor_value.astype(np.float32).flat[0]
        elif data_type == np.int32 or data_type == np.int64:
            if self._option.device == DeviceType.GPU.value or \
               self._option.device == DeviceType.CPU_GPU.value:
               data_type_arg.i = mace_pb2.DT_HALF
               value_arg.f = tensor_value.astype(np.float32).flat[0]
            else:
                data_type_arg.i = mace_pb2.DT_INT32
                value_arg.i = tensor_value.astype(np.int32).flat[0]
        else:
            mace_check(False,
                       "Not supported tensor type: %s" % data_type)
    ...
    def convert_gru(self, node):
        op = self.convert_general_op(node)
        op.type = MaceOp.Gru.name

        print('hidden_size {}, linear_before_reset {}'.format(
                node.attrs['hidden_size'], node.attrs['linear_before_reset']))

        hidden_size_arg = op.arg.add()
        hidden_size_arg.name = MaceKeyword.mace_hidden_size_str
        hidden_size_arg.i = node.attrs['hidden_size']

        linear_before_reset_arg = op.arg.add()
        linear_before_reset_arg.name = MaceKeyword.mace_linear_before_reset_str
        linear_before_reset_arg.i = node.attrs['linear_before_reset']
