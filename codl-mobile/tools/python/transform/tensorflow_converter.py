...
class TensorflowConverter(base_converter.ConverterInterface):
    ...
    def convert_ops(self, sess):
        ...
        op_idx = 0
        tf_ops = self._tf_graph.get_operations()
        for tf_op in tf_ops:
            print('Convert op %d/%d (%s)' % (op_idx, len(tf_ops), tf_op.type))
    ...
    def convert_tensors(self):
        ...
        tf_ops = self._tf_graph.get_operations()
        for tf_op in tf_ops:
            print('Convert tensor %d/%d (%s)' % (tensor_idx, len(tf_ops), tf_op.type))
    ...
    def convert_matmul(self, tf_op):
        op = self.convert_general_op(tf_op)
        # For transformer, we set target data format as NCHW
        #   to avoid extra transposing.
        #ConverterUtil.del_data_format_arg(op)
