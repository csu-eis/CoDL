...
class MatMulOpBase : public CoOperation {
 public:
  explicit MatMulOpBase(OpConstructContext *context)
      : CoOperation(context),
        transpose_a_(CoOperation::GetOptionalArg<bool>("transpose_a", false)),
        transpose_b_(CoOperation::GetOptionalArg<bool>("transpose_b", false)) {}
...
#else  // MACE_ENABLE_CODL
    mem_type_ = MemoryType::CPU_BUFFER;
    if (context->GetOpMemoryType() == MemoryType::GPU_IMAGE) {
      mem_type_ = MemoryType::GPU_IMAGE;
      kernel_ = make_unique<opencl::image::MatMulKernel>();
    } else {
      mem_type_ = MemoryType::GPU_BUFFER;
      kernel_ = make_unique<opencl::buffer::MatMulKernel>();

    }

    Workspace *ws = context->workspace();
    std::string b_name = operator_def_->input(1);
    if (ws->HasTensor(b_name)) {
      if (ws->GetTensor(b_name)->is_weight()) {
