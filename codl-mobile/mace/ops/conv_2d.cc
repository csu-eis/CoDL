...
class Conv2dOp<DeviceType::CPU, float> : public ConvPool2dOpBase {
 ...
 private:
  ...
  MaceStatus RunCpu(OpContext *context) {
    ...
    MACE_CHECK(filter->dim_size() <= 4, "filter dim size must be <= 4");
    ...
    if (conv2d_delegator_ == nullptr) {
      if (filter_h == 1 && filter_w == 1 && stride_h == 1 && stride_w == 1
        ...
        conv2d_delegator_ = make_unique<arm::fp32::Conv2dK1x1>(
          ...
          && dilation_w == 1) {
        const bool use_k1x1s2_gemm = true;
        if (use_k1x1s2_gemm) {
          conv2d_delegator_ = make_unique<arm::fp32::Conv2dK1x1S2>(
              paddings_, padding_type_);
        } else {
        ...
        conv2d_delegator_ = make_unique<arm::fp32::Conv2dK7x1S1>(
          ...
          && stride_h == 1 && stride_w == 1 && dilation_h == 1
          && dilation_w == 1) {
        conv2d_delegator_ = make_unique<arm::fp32::Conv2dK9x9S1>(
            paddings_, padding_type_);
      ...
      LOG(WARNING) << "Use general kernel"
                   ...
                   << ", padding type " << PaddingTypeToString(padding_type_)
...
#ifdef CODL_ENABLE_MACE_CONV2D_GPU
#ifdef MACE_ENABLE_OPENCL
template<>
class Conv2dGpuFloatOp : public ConvPool2dOpBase {
 public:
  explicit Conv2dOp(OpConstructContext *context)
      : ConvPool2dOpBase(context),
        activation_(ops::StringToActivationType(
 ...
 private:
  ...
  MaceStatus RunGpu(OpContext *context) override {
    const Tensor *input = this->Input(INPUT);
    const Tensor *filter = this->Input(FILTER);
    const Tensor *bias = this->InputSize() >= 3 ? this->Input(BIAS) : nullptr;
    Tensor *output = this->Output(OUTPUT);
