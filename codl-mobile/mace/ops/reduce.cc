...
#include "mace/ops/opencl/buffer/reduce.h"
...
class ReduceOp<DeviceType::CPU, T> : public ReduceOpBase {
 ...
 private:
  ...
  void Compute(const OpContext *context, const Tensor *input, Tensor *output) {
    ...
    VLOG(1) << "data_reshape " << VectorToString<int>(data_reshape_);
 ...
 public:
  explicit ReduceOp(OpConstructContext *context)
    ...
    if (context->GetOpMemoryType() == MemoryType::GPU_IMAGE) {
      ...
      kernel_ = make_unique<opencl::buffer::ReduceKernel>(reduce_type_,
                                                          axis_);
    }
  }
  MaceStatus Run(OpContext *context) override {
              ...
              [](OpConditionContext *context) -> std::set<DeviceType> {
                ...
                auto axis =
                    ProtoArgHelper::GetRepeatedArgs<OperatorDef, int>(
                        *op, "axis");
                VLOG(2) << "axis " << VectorToString<int>(axis);
                if ((axis.size() != 2 || axis[0] != 1 || axis[1] != 2) &&
                     axis.size() != 1 && axis.size() != 0) {
                  return {DeviceType::CPU};
                }
                auto tensor_shape_info = context->tensor_shape_info();
                VLOG(2) << "input0_shape_count " << tensor_shape_info->count(op->input(0))
                        << ", input0_shape_size " << tensor_shape_info->at(op->input(0)).size();
                if (tensor_shape_info->count(op->input(0)) == 0
                    || (tensor_shape_info->at(op->input(0)).size() != 4
                    && tensor_shape_info->at(op->input(0)).size() != 1)) {
                  return {DeviceType::CPU};
                }
                return {DeviceType::CPU, DeviceType::GPU};
              }));
