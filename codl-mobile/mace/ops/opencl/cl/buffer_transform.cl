...
__kernel void transform_in_out_channel(BUFFER_OUT_OF_RANGE_PARAMS
                                       GLOBAL_WORK_GROUP_SIZE_DIM2
                                       __global IN_DATA_TYPE *input,
                                       __private const int height,
                                       __private const int width,
                                       ...
                                       __global DATA_TYPE *output) {
  const int wc_blk_idx = get_global_id(0);
  const int hb_idx = get_global_id(1);

#ifndef NON_UNIFORM_WORK_GROUP
  if (wc_blk_idx >= global_size_dim0 ||
      hb_idx >= global_size_dim1) {
    return;
  }
#endif
  const int chan_blk     = (chan + 3) >> 2;
  const int batch_idx    = hb_idx / height;
  const int height_idx   = hb_idx - mul24(batch_idx, height);
  const int width_idx    = wc_blk_idx / chan_blk;
  const int chan_blk_idx = wc_blk_idx - mul24(width_idx, chan_blk);
  const int chan_idx     = chan_blk_idx << 2;
  const int offset       = mad24(mad24(mad24(batch_idx,
                              height, height_idx),
                              width, width_idx),
                              chan, chan_idx);
  DATA_TYPE4 value = 0;
  const int remain_chan = chan - chan_idx;
  if (remain_chan < 4) {
    switch (remain_chan) {
      case 3:
        value.z = CONVERT(input[offset + 2]);
      case 2:
        value.y = CONVERT(input[offset + 1]);
      case 1:
        value.x = CONVERT(input[offset]);
    }
  } else {
    value = CONVERT4(vload4(0, input + offset));
  }
...
__kernel void transform_data_type_v2(BUFFER_OUT_OF_RANGE_PARAMS
                                     __private const int global_size_dim0,
                                     __global IN_DATA_TYPE *input,
                                     __private const int input_offset,
                                     __private const int size,
                                     __global DATA_TYPE *output) {
  const int out_blk_idx = get_global_id(0);

#ifndef NON_UNIFORM_WORK_GROUP
  if (out_blk_idx >= global_size_dim0) {
    return;
  }
#endif
  const int out_idx = out_blk_idx << 2;

  const int diff = size - out_idx;
  DATA_TYPE4 input_value = 0;
  if (diff < 4) {
    switch (diff) {
      case 3:
        input_value.z = CONVERT(input[input_offset + 2]);
      case 2:
        input_value.y = CONVERT(input[input_offset + 1]);
      case 1:
        input_value.x = CONVERT(input[input_offset]);
    }
  } else {
    input_value = CONVERT4(vload4(out_idx, input + input_offset));
  }
