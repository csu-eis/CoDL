...
__kernel void conv2d_general(BUFFER_OUT_OF_RANGE_PARAMS
  ...
  const int in_offset_base = ((batch_idx * in_height + in_height_idx)
                                  * in_width + in_width_idx) * in_chan;
  //int filter_offset_base = mul24(out_chan_blk_idx, filter_in_chan) << 2;
  int filter_offset_base = (out_chan_blk_idx * filter_in_chan) << 2;
  ...
  for (int filter_h_idx = 0; filter_h_idx < filter_height; ++filter_h_idx) {
    ...
    int in_height_offset = filter_h_idx * dilated_h_offset + in_offset_base;
    for (int filter_w_idx = 0; filter_w_idx < filter_width; ++filter_w_idx) {
      ...
      int in_offset = filter_w_idx * dilated_w_offset + in_height_offset;
  ...
  int out_offset = ((batch_idx * out_height + out_height_idx)
                        * out_width + out_width_idx) * out_chan + out_chan_idx;
