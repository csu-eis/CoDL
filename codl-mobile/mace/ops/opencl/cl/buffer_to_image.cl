...
__kernel void in_out_buffer_to_image_h_offset(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global const DATA_TYPE *input, /* NHWC */
    __private const int input_offset,
    ...
    __private const int width,
    __private const int channels,
    __private const int h_offset,
    __private const int part_height,
    __private const int inner_size,  // Unused
    __write_only image2d_t output) {
  int w = get_global_id(0);
  int h = get_global_id(1);
  ...
  const int width_idx = w - mul24(channel_idx, width);
  channel_idx = channel_idx << 2;
  const int offset = mad24(mad24(mad24(batch_idx, part_height, height_idx),
                           width, width_idx),
                           channels, input_offset + channel_idx);
  ...
  int2 coord = (int2)(w, h + h_offset);
  WRITE_IMAGET(output, coord, values);
}

__kernel void in_out_buffer_to_image_n_offset_nchw(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global const DATA_TYPE *input, /* NCHW */
    __private const int input_offset,
    __private const int height,
    __private const int width,
    __private const int channels,
    __private const int n_offset,
    __private const int part_batch,
    __private const int inner_size,
    __write_only image2d_t output) {
  int w = get_global_id(0);
  int h = get_global_id(1);
  ...
  const int width_idx = w - mul24(channel_idx, width);
  channel_idx = channel_idx << 2;
  const int offset = mad24(mad24(mad24(batch_idx, channels, channel_idx),
                                 height, height_idx),
                           width, width_idx + input_offset);
  ...
  const int h_offset = mul24(n_offset, height);
  int2 coord = (int2)(w, h + h_offset);
  WRITE_IMAGET(output, coord, values);
}

__kernel void in_out_buffer_to_image_h_offset_nchw(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global const DATA_TYPE *input, /* NCHW */
    __private const int input_offset,
...
__kernel void in_out_buffer_to_image_c_offset_nchw(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global const DATA_TYPE *input, /* NCHW */
    __private const int input_offset,
    ...
    __private const int width,
    __private const int channels,
    __private const int ch_offset,
    __private const int part_channels,
    __private const int inner_size,
    __write_only image2d_t output) {
  int w = get_global_id(0);
  int h = get_global_id(1);
  ...
  const int w_offset = mul24(ch_offset >> 2, width);
  int2 coord = (int2)(w + w_offset, h);
  WRITE_IMAGET(output, coord, values);
}
...
__kernel void in_out_image_to_buffer_h_offset(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global DATA_TYPE *output, /* NHWC */
    __private const int height,
  ...
  channel_idx = channel_idx << 2;
  const int offset = mad24(mad24(mad24(batch_idx, part_height, height_idx), 
                           width, width_idx), channels, channel_idx);
...
__kernel void in_out_image_to_buffer_n_offset_nchw(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global DATA_TYPE *output, /* NCHW */
    __private const int height,
    __private const int width,
    __private const int channels,
    __private const int n_offset,
  ...
  if (size < 4) {
    switch (size) {
      case 3:
        output[offset + (inner_size << 1)] = values.s2;
      case 2:
        output[offset + inner_size]        = values.s1;
      case 1:
        output[offset]                     = values.s0;
    }
  } else {
    output[offset + 3 * inner_size] = values.s3;
    output[offset + 2 * inner_size] = values.s2;
    output[offset +     inner_size] = values.s1;
    output[offset]                  = values.s0;
  }
}

__kernel void in_out_image_to_buffer_h_offset_nchw(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global DATA_TYPE *output, /* NCHW */
    __private const int height,
...
__kernel void in_out_image_to_buffer_c_offset_nchw(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global DATA_TYPE *output, /* NCHW */
    __private const int height,
    __private const int width,
    __private const int channels,
    __private const int ch_offset, /* channels, not channel blocks */
    __private const int part_channels, /* channels, not channel blocks */
    __private const int inner_size,
    __read_only image2d_t input) {
  int w = get_global_id(0);
  int h = get_global_id(1);
  ...
  const int width_idx = w - mul24(channel_idx, width);
  channel_idx = channel_idx << 2;
  const int offset = mad24(mad24(mad24(batch_idx, part_channels, channel_idx),
                                 height, height_idx),
                           width, width_idx);
  ...
  int2 coord = (int2)(w + w_offset, h);
  DATA_TYPE4 values = READ_IMAGET(input, SAMPLER, coord);
  const int size = part_channels - channel_idx;
  if (size < 4) {
    switch (size) {
      case 3:
        output[offset + (inner_size << 1)] = values.s2;
...
__kernel void weight_width_buffer_to_image(OUT_OF_RANGE_PARAMS
  ...
  int offset =
      mad24(out_chan_idx, inner_size,
          mad24(mad24(in_chan_idx, height, height_idx),
              width, width_idx + input_offset));
...
__kernel void gemm_in_out_buffer_to_image(OUT_OF_RANGE_PARAMS
                                          GLOBAL_WORK_GROUP_SIZE_DIM2
                                          __global const DATA_TYPE *input, /* NHWC */
                                          __private const int input_offset,
                                          __private const int height,
  ...
  const int batch_idx = h / ((channels + 3) >> 2);
  const int channel_idx = (h - mul24(batch_idx, channels)) << 2;
  const int width_idx = w / height;
  const int height_idx = w - mul24(width_idx, height);
  const int offset =
      mad24(mad24(mad24(batch_idx, height, height_idx), width, width_idx),
            channels,
            input_offset + channel_idx);
...
__kernel void gemm_in_out_image_to_buffer(OUT_OF_RANGE_PARAMS
                                          GLOBAL_WORK_GROUP_SIZE_DIM2
                                          __global DATA_TYPE *output, /* NHWC */
                                          __private const int height,
                                          __private const int width,
...
__kernel void winograd_filter_buffer_to_image_2x2(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
...
// only support 3x3 now
__kernel void winograd_filter_image_to_buffer_2x2(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
...
// only support 3x3 now
__kernel void winograd_filter_buffer_to_image_6x6(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global const DATA_TYPE *input, //Oc, Ic, H, W
    __private const int input_offset,
...
__kernel void winograd_filter_image_to_buffer_6x6(
    OUT_OF_RANGE_PARAMS
    GLOBAL_WORK_GROUP_SIZE_DIM2
    __global DATA_TYPE *output, //Oc, Ic, H, W
    __private const int height,
