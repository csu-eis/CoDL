...
MaceStatus Deconv2dKernel::Compute(
    ...
    Tensor *output) {
  MACE_CHECK(output_shape.size() == 4, "deconv output shape shoud be 4-dims");

#if 0
  LOG(INFO) << "input_shape: " << VectorToString<index_t>(input->shape());
  LOG(INFO) << "filter_shape: " << VectorToString<index_t>(filter->shape());
  LOG(INFO) << "bias_shape: " << VectorToString<index_t>(bias->shape());
...
#if 0
  const std::vector<uint32_t> gwsv = {gws[0], gws[1], gws[2]};
