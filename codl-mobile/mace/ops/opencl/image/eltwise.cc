...
MaceStatus EltwiseKernel::Compute(
  ...
  if (input1 == nullptr) {
    ...
    const bool is_case0 = input0->dim_size() == input1->dim_size()
                              && input0->dim_size() == 4;
    const bool is_case1 = input0->dim_size() == 1 || input1->dim_size() == 1;
    const bool is_case2 = input0->dim_size() == 4 || input1->dim_size() == 3;
    MACE_CHECK(is_case0 || is_case1 || is_case2)
  ...
  std::vector<index_t> output_shape(input0->dim_size());
  output_shape[0] = input0->dim(0);
  if (input0->dim_size() > 1) {
    output_shape[1] = input0->dim(1);
    output_shape[2] = input0->dim(2);
  }
  if (input0->dim_size() > 3) {
  ...
  if (type_ == EltwiseType::EQUAL) {
    return MaceStatus::MACE_SUCCESS;
  }

  if (input0->dim_size() != 4) {
    return MaceStatus::MACE_SUCCESS;
  }
