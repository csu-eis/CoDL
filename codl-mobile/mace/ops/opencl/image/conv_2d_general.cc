...
std::vector<uint32_t> Conv2dLocalWS(OpenCLRuntime *runtime,
                                    const uint32_t *gws,
...
MaceStatus Conv2d(OpContext *context,
  ...
  if (!IsVecEqual(*prev_input_shape, input->shape())) {
    ...
    kernel->setArg(idx++, static_cast<uint32_t>(stride[0]));
    kernel->setArg(idx++, static_cast<uint32_t>(stride[1]));
  ...
  std::vector<uint32_t> lws =
      Conv2dLocalWS(runtime, gws, filter->dim(2) * filter->dim(3), *kwg_size);
#if 0
  int num_wraps;
