...
#define MACE_OPENCL_WIDTH_BLOCK_SIZE 4

//namespace {

std::vector<uint32_t> Conv2dK1x1LocalWS(OpenCLRuntime *runtime,
                                        const uint32_t *gws,
...
MaceStatus Conv2dK1x1(OpContext *context,
  ...
  index_t width_blocks;
  if (MACE_OPENCL_WIDTH_BLOCK_SIZE == 4)
    width_blocks = RoundUpDiv4(width);
  else
    width_blocks = RoundUpDiv<index_t, MACE_OPENCL_WIDTH_BLOCK_SIZE>(width);
  ...
  std::vector<uint32_t> lws = Conv2dK1x1LocalWS(runtime, gws, *kwg_size);
#if 0
  int num_wraps;
  CalcOpenCLWarpNumber(runtime, gws, lws.data(), &num_wraps);
  std::vector<uint32_t> gws_vector = {gws[0], gws[1], gws[2]};
  LOG(INFO) << "gws " << VectorToString<uint32_t>(gws_vector)
