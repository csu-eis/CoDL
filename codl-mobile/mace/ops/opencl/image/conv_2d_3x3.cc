...
#define MACE_OPENCL_WIDTH_BLOCK_SIZE 5

//namespace {

std::vector<uint32_t> Conv2dK3x3LocalWS(OpenCLRuntime *runtime,
                                        const uint32_t *gws,
                                        const uint32_t kwg_size,
                                        const uint32_t kwave_size = 0) {
  MACE_UNUSED(kwave_size);
  // (inputs + weights + outputs) * array_size * sizeof(float)
  //const uint32_t kernel_cache_size = (5 + 4 + 5) * 4 * 4;
  const uint32_t kernel_cache_size = (MACE_OPENCL_WIDTH_BLOCK_SIZE + 4
                                      + MACE_OPENCL_WIDTH_BLOCK_SIZE) * 4 * 4;

  std::vector<uint32_t> lws = LocalWSFromEnv();
  if (!lws.empty()) {
    return lws;
  }
...
#if 0
    // Print debug info.
    LOG(INFO) << "kwg_size " << kwg_size
              << " cache_size " << cache_size
              << " kBaseGPUMemCacheSize " << kBaseGPUMemCacheSize
              << " compute_units " << compute_units
              << " base " << base;
    const std::vector v_gws = {gws[0], gws[1], gws[2]};
    const std::vector v_lws = {lws[0], lws[1], lws[2]};
    LOG(INFO) << "gws " << VectorToString<uint32_t>(v_gws)
              << " lws " << VectorToString<uint32_t>(v_lws);
#endif
  }
  return lws;
}
...
MaceStatus Conv2dK3x3(OpContext *context,
  ...
  const index_t width_blocks =
    RoundUpDiv<index_t, MACE_OPENCL_WIDTH_BLOCK_SIZE>(width);
  ...
  if (kernel->get() == nullptr) {
    ...
    built_options.emplace("-DWIDTH_BLOCK_SIZE="
                            + std::to_string(MACE_OPENCL_WIDTH_BLOCK_SIZE));
    built_options.emplace("-DUSE_LWS");
  ...
  if (!IsVecEqual(*prev_input_shape, input->shape())) {
    ...
    kernel->setArg(idx++, stride[0]);
    kernel->setArg(idx++, stride[1]);
  ...
  std::vector<uint32_t> lws = Conv2dK3x3LocalWS(runtime, gws, *kwg_size);
//ifdef CODL_ENABLE_DEBUG_INFO
#if 0
  LOG(INFO) << "gws " << VectorToString<index_t>({gws[0], gws[1], gws[2]})
            << ", lws " << VectorToString<uint32_t>(lws)
            << ", kwg_size " << *kwg_size;
#endif
...
#undef MACE_OPENCL_WIDTH_BLOCK_SIZE
