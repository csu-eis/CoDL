...
MaceStatus Conv2dGeneral(OpContext *context,
  ...
  if (kernel->get() == nullptr) {
    ...
    VLOG(1) << "Build conv2d buffer kernel";
  ...
  MACE_UNUSED(input_changed);
  if (input_changed) {
    VLOG(1) << "Input changed for conv2d buffer kernel";
    ...
    MACE_CHECK(filter_buffer_shape.size() > 0);
    kernel->setArg(idx++,
    ...
    VLOG(1) << "ih " << in_height << ", iw " << in_width
              << ", ic " << padded_input->dim(3);
    VLOG(1) << "fh " << filter_height << ", fw " << filter_width;
    VLOG(1) << "fbs " << VectorToString<index_t>(filter_buffer_shape);
    VLOG(1) << "oh " << height << ", ow " << width << ", oc " << channel;
  }
...
#if 1
  VLOG(1) << "gws " << VectorToString<index_t>({gws[0], gws[1]})
