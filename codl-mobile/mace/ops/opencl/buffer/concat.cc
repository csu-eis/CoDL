...
MaceStatus Concat2(OpContext *context,
  ...
  if (kernel->get() == nullptr) {
    ...
    MACE_RETURN_IF_ERROR(runtime->BuildKernel("concat_buffer", kernel_name,
  ...
  if (!IsVecEqual(*prev_input_shape, input0->shape())) {
    ...
    kernel->setArg(idx++, *(input0->opencl_buffer()));
    kernel->setArg(idx++, *(input1->opencl_buffer()));
  ...
  std::string tuning_key = Concat("concat_opencl_kernel_buffer",
