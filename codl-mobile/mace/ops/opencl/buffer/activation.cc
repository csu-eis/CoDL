...
#include "mace/ops/opencl/buffer/activation.h"
...
MaceStatus ActivationKernel::Compute(
  ...
  if (kernel_.get() == nullptr) {
    ...
    switch (activation_) {
      case RELU: {
        tuning_key_prefix_ = "relu_opencl_kernel_buffer";
        built_options.emplace("-DUSE_RELU");
        break;
      }
      case RELUX: {
        tuning_key_prefix_ = "relux_opencl_kernel_buffer";
        built_options.emplace("-DUSE_RELUX");
        break;
      }
      case PRELU: {
        tuning_key_prefix_ = "prelu_opencl_kernel_buffer";
        built_options.emplace("-DUSE_PRELU");
        break;
      }
      case TANH: {
        tuning_key_prefix_ = "tanh_opencl_kernel_buffer";
        built_options.emplace("-DUSE_TANH");
        break;
      }
      case SIGMOID: {
        tuning_key_prefix_ = "sigmoid_opencl_kernel_buffer";
        built_options.emplace("-DUSE_SIGMOID");
        break;
      }
      case LEAKYRELU: {
        tuning_key_prefix_ = "leakyrelu_opencl_kernel_buffer";
    ...
    MACE_RETURN_IF_ERROR(runtime->BuildKernel("activation_buffer", kernel_name,
  ...
  if (!IsVecEqual(input_shape_, input->shape())) {
    ...
    if (activation_ == PRELU) {
      ...
      kernel_.setArg(idx++, *(alpha->opencl_buffer()));
    }
