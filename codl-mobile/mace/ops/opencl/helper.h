...
// DEFINE(fucheng): OpenCL event callback debug
//#define CODL_ENABLE_EVENT_CALLBACK_DEBUG

class OpenCLEventStatusTime {
public:
  OpenCLEventStatusTime()
      : queued_(0), submitted_(0), running_(0), complete_(0) {}

  int64_t queued() const {
    return queued_;
  }

  void SetQueued(int64_t v) {
    queued_ = v;
  }
  ...
  void SetRunning(int64_t v) {
    running_ = v;
  }

  int64_t complete() const {
    return complete_;
  }

  void SetComplete(int64_t v) {
    complete_ = v;
  }
