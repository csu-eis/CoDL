...
#if 0
    LOG(INFO) << "TransformOp: input " << input->name()
              << ", output " << output->name();
#endif
...
template<>
class BufferTransformOp<DeviceType::GPU, int32_t> : public Operation {
 public:
  explicit BufferTransformOp(OpConstructContext *context)
      : Operation(context),
        wino_blk_size_(Operation::GetOptionalArg<int>("wino_block_size", 0)),
  ...
  MaceStatus Run(OpContext *context) override {
    ...
    // NOTE(fucheng): See information of Transform
    //LOG(INFO) << "Transform buffer type " << type;
...
void RegisterBufferTransform(OpRegistryBase *op_registry) {
  ...
  MACE_REGISTER_GPU_INT32_OP(op_registry, "BufferTransform", BufferTransformOp);
}
