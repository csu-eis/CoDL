...
#include "mace/ops/opencl/buffer/eltwise.h"
...
class EltwiseOp : public Operation {
...
#if 0
    LOG(INFO) << "input0_shape " << VectorToString<index_t>(input0->shape())
              << ", input1_shape " << VectorToString<index_t>(input1->shape());
#endif
 ...
 private:
  ...
  MaceStatus DoEltwise(const OpContext *context,
...
#if 1
    VLOG(2) << "input0_shape " << VectorToString<index_t>(input0->shape())
            << ", input1_shape " << VectorToString<index_t>(input1->shape());
#endif
    ...
    if (has_data_format_) {
      // NOTE(fucheng): For input1, four cases of shape are OK.
      //                0. []; 1. [N/1,C,H,W]; 2. [C]; 3. [C,1,1].
      const bool is_case0 = input1->dim_size() == 0;
      const bool is_case1 = input1->dim_size() == 4 &&
                            input1->dim(1) == input0->dim(1) &&
                            (input1->dim(0) == input0->dim(0) || input1->dim(0) == 1);
      const bool is_case2 = input1->dim_size() == 1 &&
                            input1->dim(0) == input0->dim(1);
      const bool is_case3 = input1->dim_size() == 3 &&
                            input1->dim(0) == input0->dim(1) &&
                            input1->dim(1) == 1 && input1->dim(2) == 1;
      MACE_CHECK(input0->dim_size() == 4 &&
                 (is_case0 || is_case1 || is_case2 || is_case3),
 ...
 public:
  explicit EltwiseOp(OpConstructContext *context)
    ...
    if (context->GetOpMemoryType() == MemoryType::GPU_IMAGE) {
      ...
      kernel_ = make_unique<opencl::buffer::EltwiseKernel>(
          type, coeff, scalar_input, scalar_input_index);
    }
    // Transform filters
  ...
  MaceStatus Run(OpContext *context) override {
    ...
    if (input1 != nullptr) {
      VLOG(1) << "input1_shape " << VectorToString<index_t>(input1->shape());
    }
    Tensor *output = this->Output(0);
