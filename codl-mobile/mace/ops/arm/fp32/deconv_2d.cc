...
MaceStatus Deconv2dBase::ResizeOutAndPadOut(
...
#if 0
  LOG(INFO) << "input_out_shape " << VectorToString<index_t>(out_shape);
#endif
  ...
  CalDeconvOutputShapeAndPadSize(input->shape(),
                                 ...
                                 DataFormat::NCHW);
#if 0
  LOG(INFO) << "output_out_shape " << VectorToString<index_t>(out_shape)
            << ", padded_out_shape " << VectorToString<index_t>(padded_out_shape);
#endif
...
MaceStatus Deconv2dBase::ResizeOutAndPadOut(
  ...
  std::vector<index_t> out_shape;
  if (output_shape) {
    LOG(INFO) << "Map output shape tensor";
    // NOTE(fucheng): Should not do mapping here, use shape vector instead.
    ...
    out_shape = std::vector<index_t>(4, 0);
    MACE_CHECK(out_shape.size() == 4, "output shape should be 4-dims");
  }

  return ResizeOutAndPadOut(context,
                            input,
                            filter,
                            out_shape,
                            output,
...
MaceStatus Deconv2dBase::ResizeOutAndPadOut(
    ...
    std::vector<int> *out_pad_size,
    std::unique_ptr<Tensor> *padded_output) {
  std::vector<index_t> out_shape = output->shape();
  MACE_CHECK(out_shape.size() == 4,
             "output shape should be 4-dims, but we got ",
             VectorToString<index_t>(out_shape));
  return ResizeOutAndPadOut(context,
                            input,
                            filter,
                            out_shape,
