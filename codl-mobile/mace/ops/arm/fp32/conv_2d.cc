...
#if 0
// NOTE(fucheng): We move to mace/utils/string_util.cc.
template <typename T>
...
#if 0
  LOG(INFO) << "CalOutputShapeAndInputPadSize:"
            << " paddings_empty " << paddings_.empty();
#endif
  if (paddings_.empty()) {
#if 0
    // Debug(fucheng)
    LOG(INFO) << "CalcNCHWPaddingAndOutputSize";
#endif
...
MaceStatus Conv2dBase::ResizeOutAndPadInOut(const OpContext *context,
  ...
  // NOTE(fucheng): Use cpu device's scratch buffer
...
void Conv2dBase::PadInput(const Tensor &src,
  ...
  PadInput(src, pad_top, pad_bottom, pad_left, pad_right, dst);
}
