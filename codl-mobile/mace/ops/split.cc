...
class SplitOp<DeviceType::CPU, T> : public Operation {
 public:
  ...
  void Validate() {
    ...
    VLOG(1) << "input_shape " << VectorToString<index_t>(this->Input(0)->shape())
            << ", axis " << axis_
      ...
      OpConditionBuilder("Split")
          .SetDevicePlacerFunc(
              [](OpConditionContext *context) -> std::set<DeviceType> {
...
#if 1
                VLOG(2) << "axis " << axis
                        << ", output_shape0_dim_size " << op->output_shape(0).dims_size();
                if (op->output_shape(0).dims_size() >= 4) {
                  VLOG(2) << "output_shape0_dims3 " << op->output_shape(0).dims()[3];
                }
#endif
                ...
                // NOTE(fucheng):
                if (axis == 1 && op->output_shape(0).dims_size() == 3
                    || axis == 1) {
                  return {DeviceType::CPU, DeviceType::GPU};
                }
