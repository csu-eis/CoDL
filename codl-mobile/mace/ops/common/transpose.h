...
#define DST_DIMS_OIHW_TO_OHWI DST_DIMS_NCHW_TO_NHWC        // OIHW->OHWI
...
template<typename T>
MaceStatus PartTranspose(utils::ThreadPool *thread_pool,
                         const T *input,
                         const std::vector<int64_t> &input_shape,
                         const std::vector<int> &dst_dims,
  ...
  if (input_shape.size() == 4) {
      ...
      std::vector<index_t> idim(4, 0);
      std::vector<index_t> odim(4, 0);
      index_t odim_offset = odim_ranges[0][0] * out_stride[0] + odim_ranges[1][0] * out_stride[1]
                            + odim_ranges[2][0] * out_stride[2] + odim_ranges[3][0];
      for (odim[0] = odim_ranges[0][0]; odim[0] < odim_ranges[0][1]; ++odim[0]) {
        for (odim[1] = odim_ranges[1][0]; odim[1] < odim_ranges[1][1]; ++odim[1]) {
          for (odim[2] = odim_ranges[2][0]; odim[2] < odim_ranges[2][1]; ++odim[2]) {
            for (odim[3] = odim_ranges[3][0]; odim[3] < odim_ranges[3][1]; ++odim[3]) {
