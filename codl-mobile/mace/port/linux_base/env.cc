...
int64_t LinuxBaseEnv::NowNanos() {
  return mace::port::posix::NowNanos();
}
...
MaceStatus LinuxBaseEnv::GetCPUMaxFreq(std::vector<float> *max_freqs) {
  ...
  //LOG(INFO) << "CPU freq: " << MakeString(*max_freqs);
