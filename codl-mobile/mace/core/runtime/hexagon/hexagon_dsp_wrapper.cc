...
//#define MACE_ENABLE_HEXAGON_NODE_LIST
...
HexagonDSPWrapper::HexagonDSPWrapper() {
  ...
  //LOG(INFO) << "Hexagon DSP version: " << version;
...
int HexagonDSPWrapper::GetVersion() {
  int version;
  int ret = hexagon_nn_version(&version);
  if (ret != 0) {
    LOG(WARNING) << "Get version failed: " << ret;
  }
  MACE_CHECK(ret == 0, "get version error");
  return version;
}

bool HexagonDSPWrapper::RequestUnsignedPD() {
  LOG(INFO) << "Request unsigned PD";
...
bool HexagonDSPWrapper::SetPower(HexagonNNCornerType corner,
  ...
  //MACE_CHECK(ret == 0, "hexagon set power error");
  return ret == 0;
}

bool HexagonDSPWrapper::Config() {
  int ret = hexagon_nn_config();
  if (ret != 0) {
    LOG(WARNING) << "Hexagon config failed: " << ret;
  }
  MACE_CHECK(ret == 0, "hexagon config error");
  return true;
}

bool HexagonDSPWrapper::Init() {
  int ret = hexagon_nn_init(&nn_id_);
  if (ret != 0) {
    LOG(WARNING) << "Hexagon init failed: " << ret;
  }
  MACE_CHECK(ret == 0, "hexagon_nn_init failed");
  ResetPerfInfo();
  return true;
}
...
bool HexagonDSPWrapper::SetupGraph(const NetDef &net_def,
  ...
  for (const InputOutputInfo &input_info : net_def.input_info()) {
    ...
    VLOG(1) << "InputInfo:"
            ...
            << " type: " << input_info.data_type();

    //hexagon_nn_input empty_input_list[] = {};

    //hexagon_nn_output output_list[] = {
    //    {.rank = 4,
    //     .max_sizes = {static_cast<unsigned int>(input_shape[0]),
    //                   static_cast<unsigned int>(input_shape[1]),
    //                   static_cast<unsigned int>(input_shape[2]),
    //                   static_cast<unsigned int>(input_shape[3])},
    //     .elementsize = sizeof(float),}
    //};

    //hexagon_nn_append_node(
    //    nn_id_,             // Graph handle we're appending into
    ...
    //    output_list,        // The list of outputs from this node
...
#ifdef MACE_ENABLE_HEXAGON_NODE_LIST
  ...
  if (!const_node_list.empty()) {
    ...
    MACE_CHECK(
        hexagon_nn_append_const_node_list(nn_id_, const_node_list.data(),
                                          const_node_list.size()) == 0,
        "append const node error");
#else  // MACE_ENABLE_HEXAGON_NODE_LIST
    int const_node_count = 0;
    for (auto iter = const_node_list.begin();
        iter != const_node_list.end(); ++iter, ++const_node_count) {
      if (const_node_count >= 1024) {
        break;
      }
      auto &node = *iter;
      VLOG(1) << "ConstNodeInfo: nn_id " << nn_id_
              << ", node_id " << static_cast<uint32_t>(node.node_id)
              << ", batches " << node.tensor.batches
              << ", height " << node.tensor.height
              << ", width " << node.tensor.width
              << ", depth " << node.tensor.depth
              << ", dataLen " << node.tensor.dataLen;
      MACE_CHECK(
          hexagon_nn_append_const_node(nn_id_,
                                       ...
                                       node.tensor.depth,
                                       node.tensor.data,
                                       node.tensor.dataLen) == 0,
          "append const node error");
    }
#endif  // MACE_ENABLE_HEXAGON_NODE_LIST
  }
  const_node_list.clear();
  ...
  for (const OperatorDef &op : net_def.op()) {
    ...
    //MACE_CHECK(op.node_input().size() > 0, "node input size: ", op.node_input().size());
    ...
    for (int i = 0; i < op.output_shape().size(); ++i) {
      ...
      if (i < op.output_type().size()) {
        outputs[i].elementsize = GetEnumTypeSize(
            static_cast<DataType>(op.output_type()[i]));
      } else {
        outputs[i].elementsize = GetEnumTypeSize(DataType::DT_UINT8);
      }
    ...
    VLOG(1) << "OperatorMapping: type " << op.type()
            << ", op_id " << op_id
            << ", node_id " << op.node_id();
...
#else  // MACE_ENABLE_HEXAGON_NODE_LIST
    int op_node_count = 0;
    for (auto iter = op_node_list.begin();
        iter != op_node_list.end(); ++iter, ++op_node_count) {
      if (op_node_count >= 128) {
        break;
      }
      auto &node = *iter;
      PrintOpNodeInfo(node);
      MACE_CHECK(
          hexagon_nn_append_node(nn_id_,
                                 node.node_id,
                                 node.operation,
                                 node.padding,
                                 node.inputs,
  ...
  for (const InputOutputInfo &output_info : net_def.output_info()) {
    ...
    //output_info_.emplace_back(output_info.name(),
    //                          input_info_[0].shape,
    //                          input_info_[0].data_type);

    //hexagon_nn_input input_list[] = {
    //    { .src_id = 0x1000, .output_idx = 0, },
    //};
  ...
  int ret = hexagon_nn_prepare(nn_id_);
  ...
  if (ret == 0) {
    ...
    VLOG(1) << "prepare success";
  } else {
    PrintLog();
    MACE_CHECK(ret == 0, "hexagon_nn_prepare failed");
  }
...
#define MACE_PRINT_BUFSIZE (128 * 1024 * 1024)

void HexagonDSPWrapper::PrintLog() {
  char *buf;
  if ((buf = new char[MACE_PRINT_BUFSIZE]) == NULL) return;
  int ret = hexagon_nn_getlog(nn_id_, reinterpret_cast<unsigned char *>(buf),
                              MACE_PRINT_BUFSIZE);
  if (ret == 0) {
    LOG(INFO) << std::string(buf);
  } else {
    //MACE_CHECK(ret == 0, "print log error");
    LOG(WARNING) << "print log error";
  }
...
void HexagonDSPWrapper::PrintOpNodeInfo(hexagon_nn_op_node &node) {
  LOG(INFO) << "NodeInfo: nn_id " << nn_id_
            << ", node_id " << static_cast<uint32_t>(node.node_id)
            << ", operation " << node.operation
            << ", padding " << node.padding
            << ", inputsLen " << node.inputsLen
            << ", outputsLen " << node.outputsLen;
}
