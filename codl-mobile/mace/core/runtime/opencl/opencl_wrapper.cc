...
class OpenCLLibrary final {
 ...
 public:
  ...
  using clEnqueueBarrierWithWaitListFunc = cl_int (*)(cl_command_queue,
                                                      cl_uint,
                                                      const cl_event *,
                                                      cl_event *);

  using clEnqueueMarkerWithWaitListFunc = cl_int (*)(cl_command_queue,
                                                     cl_uint,
                                                     const cl_event *,
                                                     cl_event *);

  using clSetEventCallbackFunc = cl_int (*)(
      cl_event, cl_int, void (CL_CALLBACK *) (cl_event, cl_int, void *), void *);

  using clCreateUserEventFunc = cl_event (*)(cl_context, cl_int *);

  using clSetUserEventStatusFunc = cl_int (*)(cl_event, cl_int);
...
#define MACE_CL_DEFINE_FUNC_PTR(func) func##Func func = nullptr
  ...
  MACE_CL_DEFINE_FUNC_PTR(clEnqueueBarrierWithWaitList);
  MACE_CL_DEFINE_FUNC_PTR(clEnqueueMarkerWithWaitList);
  MACE_CL_DEFINE_FUNC_PTR(clSetEventCallback);
  MACE_CL_DEFINE_FUNC_PTR(clCreateUserEvent);
  MACE_CL_DEFINE_FUNC_PTR(clSetUserEventStatus);
...
#define MACE_CL_ASSIGN_FROM_DLSYM(func)                          \
  ...
  MACE_CL_ASSIGN_FROM_DLSYM(clEnqueueBarrierWithWaitList);
  MACE_CL_ASSIGN_FROM_DLSYM(clEnqueueMarkerWithWaitList);
  MACE_CL_ASSIGN_FROM_DLSYM(clSetEventCallback);
  MACE_CL_ASSIGN_FROM_DLSYM(clCreateUserEvent);
  MACE_CL_ASSIGN_FROM_DLSYM(clSetUserEventStatus);
...
clEnqueueBarrierWithWaitList(cl_command_queue command_queue,
                             ...
                             const cl_event *event_wait_list,
                             cl_event *event) {
  auto func = mace::runtime::OpenCLLibrary::Get()->clEnqueueBarrierWithWaitList;
  if (func != nullptr) {
    MACE_LATENCY_LOGGER(3, "clEnqueueBarrierWithWaitList");
...
clEnqueueMarkerWithWaitList(cl_command_queue command_queue,
                            ...
                            const cl_event *event_wait_list,
                            cl_event *event) {
  auto func = mace::runtime::OpenCLLibrary::Get()->clEnqueueMarkerWithWaitList;
  if (func != nullptr) {
    MACE_LATENCY_LOGGER(3, "clEnqueueMarkerWithWaitList");
    return func(command_queue, num_events_in_wait_list, event_wait_list, event);
  } else {
    return CL_INVALID_PLATFORM;
  }
...
clSetEventCallback(cl_event event,
                   ...
                   void (CL_CALLBACK *func_name) (cl_event event,
                                                  cl_int status,
                                                  void *data),
                   void *data) {
  auto func = mace::runtime::OpenCLLibrary::Get()->clSetEventCallback;
  if (func != nullptr) {
    MACE_LATENCY_LOGGER(3, "clSetEventCallback");
    return func(event, callback_type, func_name, data);
  } else {
    return CL_INVALID_PLATFORM;
  }
}
...
clCreateUserEvent(cl_context context,
                  cl_int *errcode_ret) {
  auto func = mace::runtime::OpenCLLibrary::Get()->clCreateUserEvent;
  if (func != nullptr) {
    MACE_LATENCY_LOGGER(3, "clCreateUserEvent");
...
clSetUserEventStatus(cl_event event,
  ...
  auto func = mace::runtime::OpenCLLibrary::Get()->clSetUserEventStatus;
  if (func != nullptr) {
    MACE_LATENCY_LOGGER(3, "clSetUserEventStatus");
