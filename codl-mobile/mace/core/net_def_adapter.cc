...
MaceStatus NetDefAdapter::AdaptDevice(OpConditionContext *context,
  ...
  VLOG(2) << "Adapt device for op " << op_def->name()
          << " (" << op_def->type() << ")";
  ...
  if (target_device_type != DeviceType::CPU) {
    ...
    device_type = net_optimizer_.SelectBestDevice(op_def,
                                                  ...
                                                  producer_devices);
#if 1
    VLOG(2) << "device_type " << static_cast<int>(device_type)
            << ", target_device_type " << static_cast<int>(target_device_type)
            << ", available_devices " << SetToString<DeviceType>(available_devices)
            << ", producer_devices " << VectorToString<DeviceType>(producer_devices);
#endif
...
MaceStatus NetDefAdapter::AdaptDataFormat(
  ...
  VLOG(1) << "OP data format " << static_cast<int>(op_data_format);
  ...
  if (op_def->device_type() == DeviceType::GPU
      && op_data_format == DataFormat::NCHW) {
    op_data_format = DataFormat::NHWC;
  }
  VLOG(1) << "OP adjusted data format " << static_cast<int>(op_data_format);
  *op_output_df = op_data_format;
  ...
  if (op_def->device_type() == DeviceType::GPU) {
    ...
    //target_mem_type = MemoryType::GPU_IMAGE;
  }
  ...
  for (int i = 0; i < input_size; ++i) {
    ...
    if (op_def->device_type() == DeviceType::GPU && dst_df == DataFormat::NCHW) {
      continue;
    }
    if (src_df != DataFormat::NONE
      ...
      if (transformed_set->count(transformed_name) == 0) {
        ...
        DataType dst_dt = input_info.dtype;
        if (ws_->gpu_precision() == GPUPrecision::GPU_PRECISION_FP32) {
          dst_dt = DataType::DT_FLOAT;
        } else if (ws_->gpu_precision() == GPUPrecision::GPU_PRECISION_FP16) {
          dst_dt = DataType::DT_HALF;
        }
        VLOG(1) << "transpose op data type " << static_cast<int>(input_info.dtype);
        ...
        BuildTransposeOpDef(
            ...
            dst_dims,
            dst_dt,
            DeviceType::CPU,  // static_cast<DeviceType>(op_def->device_type())
            transpose_op_def);
        // set data format arg
...
MaceStatus NetDefAdapter::AdaptMemoryType(
    ...
    NetDef *target_net_def) {
  VLOG(2) << "Adapt memory type for op " << op_def->name();
  ...
  for (int i = 0; i < input_size; ++i) {
    ...
    VLOG(1) << "input_dtype " << static_cast<int>(input_info.dtype)
            << ", wanted_input_dtype " << static_cast<int>(wanted_input_dtype);
    if (src_mem_type != dst_mem_type ||
      ...
      if (transformed_set->count(transformed_name) == 0) {
        VLOG(1) << "Add Transform operation " << op_def->name()
                ...
                << dst_mem_type;
#if 0
        // NOTE(fucheng): See information of transfroming.
        LOG(INFO) << "Add Transform operation " << op_def->name()
                  << " to transform tensor "
                  << op_def->input(i) << ", from memory type "
                  << input_info.mem_type << " to "
                  << dst_mem_type << ", input dtype "
                  << input_info.dtype << " wanted input dtype "
                  << wanted_input_dtype;
#endif
