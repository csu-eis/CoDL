...
MemoryBlock MemoryOptimizer::CreateMemoryBlock(
  ...
  VLOG(2) << "shape " << VectorToString<int64_t>(shape);
  MemoryBlock block;
#ifdef MACE_ENABLE_OPENCL
  if (mem_type == MemoryType::GPU_IMAGE) {
    ...
    if (shape.size() == 1) {
      ...
      MACE_CHECK(shape.size() == 4) << "GPU only support 1D/2D/3D/4D/5D input"
                                    << ", but got " << shape.size();
    }

    OpenCLUtil::CalImage2DShape(shape, buffer_type, &image_shape);
#if 0
    LOG(INFO) << "CalImage2DShape: name " << op_def->name()
              <<", shape " << VectorToString<int64_t>(shape)
              << ", buffer_type " << static_cast<int>(buffer_type)
