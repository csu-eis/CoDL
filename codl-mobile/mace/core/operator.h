...
enum OpPosition {
  OP_POSITION_NONE = 0,
  OP_POSITION_HEAD = 1,
  OP_POSITION_MIDDLE = 2,
  OP_POSITION_TAIL = 3
};

class OpChainContext {
 public:
  OpChainContext() : position_(OP_POSITION_NONE), chain_(nullptr) {}

  inline void set_position(const OpPosition position) {
    position_ = position;
  }

  inline OpPosition position() const {
    return position_;
  }

  inline void set_chain(void *chain) {
    chain_ = chain;
  }
...
class Operation {
 public:
  ...
  inline OpChainContext *chain_context() {
...
#define MACE_REGISTER_GPU_INT32_OP(op_registry, op_type, class_name) \
  op_registry->Register(                                       \
      op_type,                                                 \
      DeviceType::GPU,                                         \
      DT_INT32,                                                \
      OpRegistryBase::DefaultCreator<class_name<DeviceType::GPU, int32_t>>)

#else
#define MACE_REGISTER_GPU_OP(op_registry, op_type, class_name)
#endif

// NOTE(fucheng): Add register for CPUGPU
#ifdef MACE_ENBALE_OPENCL
#define MACE_REGISTER_CPUGPU_OP(op_registry, op_type, class_name) \
  op_registry->Register(                                          \
      op_type,                                                    \
      DeviceType::GPU,                                            \
      DT_FLOAT,                                                   \
      OpRegistryBase::DefaultCreator<class_name<DeviceType::CPUGPU, float>>)
#else
#define MACE_REGISTER_CPUGPU_OP(op_registry, op_type, class_name)
#endif
