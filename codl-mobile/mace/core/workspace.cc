  ...
  if (model_data_size > 0) {
    ...
    if (diffused_buffer_) {
      for (auto &const_tensor : net_def.tensors()) {
        ...
        // LOG(fucheng): Tensor information
        /***
        LOG(INFO) << "Tensor name: " << const_tensor.name()
                  << ", data type: " << const_tensor.data_type() << ", shape: "
                  << MakeString(std::vector<index_t>(const_tensor.dims().begin(),
                                                     const_tensor.dims().end())); ***/
      ...
      for (auto &const_tensor : net_def.tensors()) {
        ...
        VLOG(3) << "Tensor name: " << const_tensor.name()
                ...
                << MakeString(std::vector<index_t>(const_tensor.dims().begin(),
                                                   const_tensor.dims().end()));
        // LOG(fucheng): Weight tensor information
        /***
        LOG(INFO) << "Tensor name: " << const_tensor.name()
                  << ", data type: " << DataTypeToString(const_tensor.data_type())
                  << ", shape: " << MakeString(std::vector<index_t>(
                      const_tensor.dims().begin(), const_tensor.dims().end())); ***/
        std::vector<index_t> dims;
        for (const index_t d : const_tensor.dims()) {
          dims.push_back(d);
        }
...
#define MACE_ENABLE_OPENCL_INT8
...
MaceStatus Workspace::PreallocateOutputTensor(
...
#ifdef MACE_ENABLE_OPENCL
  LOG(INFO) << "OpenCLFPprecision: "
            << GetGPUPrecisionName(gpu_precision_);
#endif

#ifdef MACE_ENABLE_OPENCL_FP16
  LOG(INFO) << "OpenCLImageFloatingPointPrecision: FP16";
#endif
#ifdef MACE_ENABLE_OPENCL_FP32
  LOG(INFO) << "OpenCLImageFloatingPointPrecision: FP32";
#endif

  auto &mem_blocks = mem_optimizer->mem_blocks();
  for (auto &mem_block : mem_blocks) {
    VLOG(2) << "Preallocate memory block. id: " << mem_block.mem_id()
            << ", memory type: " << mem_block.mem_type()
            << ", size: " << mem_block.x() << "x" << mem_block.y();
    // LOG(fucheng): Input/output tensor information
    /***
    LOG(INFO) << "Preallocate memory block. id: " << mem_block.mem_id()
              << ", memory type: " << mem_block.mem_type()
              << ", size: " << mem_block.x() << "x" << mem_block.y()
              << ", data type: " << DataTypeToString(mem_block.data_type()); ***/
    if (mem_block.mem_type() == MemoryType::CPU_BUFFER) {
      ...
      DataType mem_data_type = mem_block.data_type();
#ifdef MACE_ENABLE_OPENCL
      // if(!strcmp(gpu_dprec, "32"))
      if (gpu_precision_ == GPUPrecision::GPU_PRECISION_FP32) {
        if (mem_data_type == DataType::DT_HALF) {
          // MODIFY(fucheng): Change to float type
          mem_data_type = DataType::DT_FLOAT;
          /***
          LOG(INFO) << "Set memory data type from "
                    << DataTypeToString(mem_block.data_type())
                    << " to " << DataTypeToString(mem_data_type); ***/
        }
      }
#endif  // MACE_ENABLE_OPENCL
      MACE_RETURN_IF_ERROR(image_buf->Allocate(
          {static_cast<size_t>(mem_block.x()),
           static_cast<size_t>(mem_block.y())}, mem_data_type));
      preallocated_allocator_.SetBuffer(mem_block.mem_id(),
                                        std::move(image_buf));
  ...
  for (auto &tensor_mem : mem_optimizer->tensor_mem_map()) {
    DataType mem_data_type = tensor_mem.second.data_type;
#ifdef MACE_ENABLE_OPENCL
    // if(!strcmp(gpu_dprec, "32"))
    if (gpu_precision_ == GPUPrecision::GPU_PRECISION_FP32) {
      if (mem_data_type == DataType::DT_HALF) {
    ...
    // NOTE(fucheng): Add allocator in tensor.
    tensor->set_allocator(device->allocator());
...
#if 0
        // NOTE(fucheng): Show tensor information.
        LOG(INFO) << "Tensor: " << tensor_mem.first
                  << ", Mem: " << tensor_mem.second.mem_id
                  << ", Data type: " << DataTypeToString(tensor->dtype())
                  << ", Image shape: "
                  << tensor->UnderlyingBuffer()->shape()[0]
                  << ", "
                  << tensor->UnderlyingBuffer()->shape()[1];
