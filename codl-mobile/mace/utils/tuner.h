...
class Tuner {
 ...
 private:
      ...
      if (ofs.is_open()) {
        ...
        for (auto &kp : param_table_) {
          ...
          VLOG(3) << "Write tuning param: " << kp.first.c_str() << ": "
                  << MakeString(params);
          LOG(INFO) << "Write tuning param: " << kp.first.c_str() << ": "
                    << MakeString(params);
        ...
        LOG(WARNING) << "Write run parameter file failed, path " << path_;
      }
    }
  }
  ...
  void ReadRunParamters() {
    if (!IsTuning() && !tuned_param_file_path_.empty()) {
  ...
  RetType Tune(const std::function<std::vector<std::vector<param_type>>()>
    ...
    for (auto param : params) {
      ...
      LOG(INFO) << "Tuning"
                << " param " << VectorToString<param_type>(param)
                << " time " << tmp_time
                << " opt_param " << VectorToString<param_type>(*opt_params)
                << " opt_time " << opt_time;
    }
    return res;
  }
